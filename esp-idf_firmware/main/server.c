...
static void handleData() {
  ...
  if ((params[0] == 0 && count == 5) || (params[0] == 25 && count == 2)) {
    ...
    if (params[0] == 0) {
      for (int chann = 0; chann < 4; chann++) {
        if (params[1] & (1 << chann)) {
          ESP_LOGI("DSP", "Set LowPass Request %d, %d, %d %d", chann, params[2],
                   params[3], params[4]);
      ...
      ESP_LOGI("DSP", "LowPass Request CH:%d, Ord.:%d, Freq: %d, Q:%f",
               params[1], tempdata[0], tempdata[1], (float)tempdata[2] / 10.0f);
      sprintf(rx_buffer, "!,25,%d,%d,%d,%d,?", params[1], tempdata[0],
  ...
  if ((params[0] == 1 && count == 5) || (params[0] == 26 && count == 2)) {
    ...
    if (params[0] == 1) {
      ...
      for (int chann = 0; chann < 4; chann++) {
        if (params[1] & (1 << chann)) {
          ESP_LOGI("DSP", "Set HighPass Request %d, %d, %d %d", chann,
      ...
      ESP_LOGI("DSP", "HighPass Request CH:%d, Ord.:%d, Freq: %d, Q:%f",
               params[1], tempdata[0], tempdata[1], (float)tempdata[2] / 10.0f);
      sprintf(rx_buffer, "!,26,%d,%d,%d,%d,?", params[1], tempdata[0],
              tempdata[1], tempdata[2]);
    }
  }
  ...
  if ((params[0] == 2 && count == 6) || (params[0] == 27 && count == 3)) {
    ...
    if (params[0] == 2) {
      ...
      for (int chann = 0; chann < 4; chann++) {
        if (params[1] & (1 << chann)) {
          ESP_LOGI("DSP", "Set EQ Request %d %d %d %d %d", chann, params[2],
      ...
      ESP_LOGI("DSP", "EQ Request CH:%d, No.:%d, Freq: %d, Gain: %d, Q:%d",
               params[1], params[2], tempdata[0], tempdata[1], tempdata[2]);
      sprintf(rx_buffer, "!,27,%d,%d,%d,%d,%d,?", params[1], params[2],
  ...
  if ((params[0] == 3 && count == 5) || (params[0] == 28 && count == 1)) {
    ...
    if (params[0] == 3) {
      ESP_LOGI("DSP", "Set Gain Request %d %d %d %d", params[1], params[2],
               params[3], params[4]);
      SetGain(params[1], params[2], params[3], params[4]);
      sprintf(rx_buffer, "!,3,1,?");
    } else {
      GetGain(&tempdata[0], &tempdata[1], &tempdata[2], &tempdata[3]);
      ESP_LOGI("DSP", "GAIN Request: %d, %d, %d, %d", tempdata[0], tempdata[1],
               tempdata[2], tempdata[3]);
      sprintf(rx_buffer, "!,28,%d,%d,%d,%d,?", tempdata[0], tempdata[1],
              tempdata[2], tempdata[3]);
    }
  }
  ...
  if ((params[0] == 4 && count == 9) || (params[0] == 29 && count == 1)) {
    ...
    if (params[0] == 4) {
      ESP_LOGI("DSP", "Set Limiter Request %d %d %d %d %d %d %d %d", params[1],
               params[2], params[3], params[4], params[5], params[6], params[7],
               params[8]);
      SetLimiter(params[1], params[2], params[3], params[4], params[5],
                 params[6], params[7], params[8]);
      sprintf(rx_buffer, "!,4,1,?");
    } else {
      GetLimiter(&tempdata[0], &tempdata[1], &tempdata[2], &tempdata[3],
                 &tempdata[4], &tempdata[5], &tempdata[6], &tempdata[7]);
      ESP_LOGI("DSP", "LIMITER Request %d, %d, %d, %d, %d, %d, %d, %d",
               tempdata[0], tempdata[1], tempdata[2], tempdata[3], tempdata[4],
               tempdata[5], tempdata[6], tempdata[7]);
      sprintf(rx_buffer, "!,29,%d,%d,%d,%d,%d,%d,%d,%d,?", tempdata[0],
              tempdata[1], tempdata[2], tempdata[3], tempdata[4], tempdata[5],
  ...
  if (params[0] == 30 && count == 1) {
    ...
    if (irvars[0] == 0) {
      sprintf(rx_buffer, "!,55,%d,%d,%d,%d,%d,%d,%d,%d,%d,?", tempdata[0],
              tempdata[1], tempdata[2], tempdata[3], tempdata[4], tempdata[5],
              tempdata[6], tempdata[7], dbgainint);
      ESP_LOGI("DSP", "Levels: %d,%d,%d,%d Limiters: %d,%d,%d,%d Dyngain: %f",
               tempdata[0], tempdata[1], tempdata[2], tempdata[3], tempdata[4],
               tempdata[5], tempdata[6], tempdata[7], dbgain);
    } else {
      sprintf(rx_buffer, "!,555,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,?",
              tempdata[0], tempdata[1], tempdata[2], tempdata[3], tempdata[4],
              tempdata[5], tempdata[6], tempdata[7], dbgainint, irvars[1],
              irvars[2]);
      irvars[0] = 0;
      ESP_LOGI(
          "DSP",
          "Levels: %d,%d,%d,%d Limiters: %d,%d,%d,%d Dyngain: %f IR: %d,%d",
          tempdata[0], tempdata[1], tempdata[2], tempdata[3], tempdata[4],
          tempdata[5], tempdata[6], tempdata[7], dbgain, irvars[1], irvars[2]);
    }
  }
  ...
  if ((params[0] == 6 && count == 5) || (params[0] == 31 && count == 1)) {
    ...
    if (params[0] == 6) {
      ESP_LOGI("DSP", "Set Mute Request %d %d %d %d", params[1], params[2],
               params[3], params[4]);
      SetMute(params[1], params[2], params[3], params[4]);
      sprintf(rx_buffer, "!,6,1,?");
    } else {
      GetMute(&tempdata[0], &tempdata[1], &tempdata[2], &tempdata[3]);
      ESP_LOGI("DSP", "Mute: %d, %d, %d, %d", tempdata[0], tempdata[1],
               tempdata[2], tempdata[3]);
      sprintf(rx_buffer, "!,31,%d,%d,%d,%d,?", tempdata[0], tempdata[1],
              tempdata[2], tempdata[3]);
    }
  }
  ...
  if ((params[0] == 7 && count == 5) || (params[0] == 32 && count == 1)) {
    ...
    if (params[0] == 7) {
      ESP_LOGI("DSP", "Set Polarity Request %d %d %d %d", params[1], params[2],
               params[3], params[4]);
      ...
      ESP_LOGI("DSP", "Polarity: %d, %d, %d, %d", tempdata[0], tempdata[1],
               tempdata[2], tempdata[3]);
      sprintf(rx_buffer, "!,32,%d,%d,%d,%d,?", tempdata[0], tempdata[1],
              tempdata[2], tempdata[3]);
    }
  }
  ...
  if ((params[0] == 13 && count == 5) || (params[0] == 38 && count == 1)) {
    ...
    if (params[0] == 13) {
      ESP_LOGI("DSP", "Set Source Request %d %d %d %d", params[1], params[2],
               params[3], params[4]);
      SetSource(params[1], params[2], params[3], params[4]);
      sprintf(rx_buffer, "!,13,1,?");
    } else {
      GetSource(&tempdata[0], &tempdata[1], &tempdata[2], &tempdata[3]);
      ESP_LOGI("DSP", "Source Request: %d, %d, %d, %d", tempdata[0],
               tempdata[1], tempdata[2], tempdata[3]);
      sprintf(rx_buffer, "!,38,%d,%d,%d,%d,?", tempdata[0], tempdata[1],
              tempdata[2], tempdata[3]);
    }
  }
  ...
  if ((params[0] == 8 && count == 5) || (params[0] == 33 && count == 1)) {
    ...
    if (params[0] == 8) {
      ESP_LOGI("DSP", "Set Delay Request %d %d %d %d", params[1], params[2],
               params[3], params[4]);
      SetDelay(params[1], params[2], params[3], params[4]);
      sprintf(rx_buffer, "!,8,1,?");
    } else {
      GetDelay(&tempdata[0], &tempdata[1], &tempdata[2], &tempdata[3]);
      ESP_LOGI("DSP", "Delay Request: %d, %d, %d, %d", tempdata[0], tempdata[1],
               tempdata[2], tempdata[3]);
      sprintf(rx_buffer, "!,33,%d,%d,%d,%d,?", tempdata[0], tempdata[1],
              tempdata[2], tempdata[3]);
    }
  }
  ...
  if ((params[0] == 14 && count == 6) || (params[0] == 39 && count == 1)) {
    ...
    if (params[0] == 14) {
      ...
      GetDynBass(&tempdata[0], &tempdata[1], &tempdata[2], &tempdata[3],
                 &tempdata[4]);
      ESP_LOGI("DSP", "DynBass Request: %d, %d, %d, %d, %d", tempdata[0],
               tempdata[1], tempdata[2], tempdata[3], tempdata[4]);
      sprintf(rx_buffer, "!,39,%d,%d,%d,%d,%d,?", tempdata[0], tempdata[1],
  ...
  if ((params[0] == 41 && count == 3) || (params[0] == 66 && count == 1)) {
    ...
    if (params[0] == 41) {
      ...
      GetGlobalBypass(&tempdata[0], &tempdata[1], &tempdata[2], &tempdata[3],
                      &tempdata[4]);
      ESP_LOGI("DSP", "Global Bypass Request: %d, %d", tempdata[0],
               tempdata[1]);
    ...
    if (params[0] == 40) {
      for (int chann = 0; chann < 4; chann++) {
        if (params[1] & (1 << chann)) {
          ...
          ESP_LOGI("DSP", "Set ByPass Channel:%d %d %d %d %d %d %d %d", chann,
                   params[2], params[3], params[4], params[5], params[6],
                   params[7], params[8]);
          SetChannelBypass(chann, params[2], params[3], params[4], params[5],
                           params[6], params[7], params[8]);
        }
      }
      sprintf(rx_buffer, "!,40,1,?");
    } else {
      GetChannelBypass(params[1], &tempdata[0], &tempdata[1], &tempdata[2],
                       &tempdata[3], &tempdata[4], &tempdata[5], &tempdata[6]);
      ESP_LOGI("DSP", "Channel Bypass Request: %d %d %d %d %d %d %d %d",
               params[1], tempdata[0], tempdata[1], tempdata[2], tempdata[3],
               tempdata[4], tempdata[5], tempdata[6]);
      sprintf(rx_buffer, "!,65,%d,%d,%d,%d,%d,%d,%d,%d,?", params[1],
              tempdata[0], tempdata[1], tempdata[2], tempdata[3], tempdata[4],
  ...
  if ((params[0] == 12 && count == 9) || (params[0] == 37 && count == 1)) {
    ...
    if (params[0] == 12) {
      ESP_LOGI("DSP", "Set IR Request %d %d %d %d %d %d %d %d", params[1],
               params[2], params[3], params[4], params[5], params[6], params[7],
               params[8]);
      SetIRParams(params[1], params[2], params[3], params[4], params[5],
                  params[6], params[7], params[8]);
      sprintf(rx_buffer, "!,12,1,?");
    } else {
      GetIRParams(&tempdata[0], &tempdata[1], &tempdata[2], &tempdata[3],
                  &tempdata[4], &tempdata[5], &tempdata[6], &tempdata[7]);
      ESP_LOGI("DSP", "IR Params: %d, %d, %d, %d, %d, %d, %d, %d", tempdata[0],
               tempdata[1], tempdata[2], tempdata[3], tempdata[4], tempdata[5],
               tempdata[6], tempdata[7]);
      sprintf(rx_buffer, "!,37,%d,%d,%d,%d,%d,%d,%d,%d,?", tempdata[0],
              tempdata[1], tempdata[2], tempdata[3], tempdata[4], tempdata[5],
              tempdata[6], tempdata[7]);
    }
  }
