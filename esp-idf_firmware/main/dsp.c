...
signed int delaybuffers[4][delaysize];
...
void SetDynBass(int watchtime, int threshold, int frequency, int gain,
                int gainspeed) {

  if (watchtime < 10 || watchtime > 5000)
    return;
  if (threshold < 0)
    return;
  if (frequency < 30 || frequency > 200)
    return;
  if (gain < 0 || gain > 100)
    return;
  if (gainspeed < 1 || gainspeed > 100)
    return;
  ...
  dynbass_watchtime =
      (int)round((((float)watchtime) / 1000.0f) / buffertimequanta);
  ...
  ESP_LOGI("dynbass", "wt: %d, th: %f, freq: %d, gain: %f, gainspeed: %f",
           dynbass_watchtime, dynbass_threshold, frequency,
           dynbass_desired_gain, dynbass_gainspeed);
}

void GetDynBass(int *watchtime, int *threshold, int *frequency, int *gain,
                int *gainspeed) {
...
void SetSource(int channel0, int channel1, int channel2, int channel3) {
  ...
  if (channel0 < 0 || channel0 > 2)
    return;
  if (channel1 < 0 || channel1 > 2)
    return;
  if (channel2 < 0 || channel2 > 2)
    return;
  if (channel3 < 0 || channel3 > 2)
    return;
...
void GetDynBassGain(float *gain) { *gain = dynbass_gain; }
void GetLevelActive(int *r_ch0, int *r_ch1, int *r_ch2, int *r_ch3) {

  if (peak_detect[0] > limiter_threshold[0])
    *r_ch0 = settings.par_limiter_threshold[0];
  else
    *r_ch0 = peak_detect[0];

  if (peak_detect[1] > limiter_threshold[1])
    *r_ch1 = settings.par_limiter_threshold[1];
  else
    *r_ch1 = peak_detect[1];

  if (peak_detect[2] > limiter_threshold[2])
    *r_ch2 = settings.par_limiter_threshold[2];
  else
    *r_ch2 = peak_detect[2];

  if (peak_detect[3] > limiter_threshold[3])
    *r_ch3 = settings.par_limiter_threshold[3];
  else
    *r_ch3 = peak_detect[3];
}
...
void GetLimiterActive(int *r_ch0, int *r_ch1, int *r_ch2, int *r_ch3) {
  ...
  if (limiter_timeout[0] > 0)
    *r_ch0 = 1;
  else
    *r_ch0 = 0;

  if (limiter_timeout[1] > 0)
    *r_ch1 = 1;
  else
    *r_ch1 = 0;

  if (limiter_timeout[2] > 0)
    *r_ch2 = 1;
  else
    *r_ch2 = 0;

  if (limiter_timeout[3] > 0)
    *r_ch3 = 1;
  else
    *r_ch3 = 0;
}

void GetLimiter(int *thres_ch0, int *thres_ch1, int *thres_ch2, int *thres_ch3,
                int *rel_ch0, int *rel_ch1, int *rel_ch2, int *rel_ch3) {
...
void SetLimiter(int thres_ch0, int thres_ch1, int thres_ch2, int thres_ch3,
                int rel_ch0, int rel_ch1, int rel_ch2, int rel_ch3) {
  ...
  for (int i = 0; i < 4; i++)
    limiter_threshold[i] = (float)settings.par_limiter_threshold[i];
}
...
void SetGain(int channel0, int channel1, int channel2, int channel3) {
  ...
  if (channel0 < -20 || channel0 > 20)
    return;
  if (channel1 < -20 || channel1 > 20)
    return;
  if (channel2 < -20 || channel2 > 20)
    return;
  if (channel3 < -20 || channel3 > 20)
    return;
...
void PrintParameters() {
  ESP_LOGE("PAR", "Source: %d %d %d %d", settings.sourceselect[0],
           settings.sourceselect[1], settings.sourceselect[2],
           settings.sourceselect[3]);
  ESP_LOGE("PAR", "MUTE: %d %d %d %d", settings.mute[0], settings.mute[1],
           settings.mute[2], settings.mute[3]);
  ESP_LOGE("PAR", "POL: %d %d %d %d", settings.polarity[0],
           settings.polarity[1], settings.polarity[2], settings.polarity[3]);
  ESP_LOGE("PAR", "GAINS: %d %d %d %d", settings.par_channel_gains[0],
           settings.par_channel_gains[1], settings.par_channel_gains[2],
           settings.par_channel_gains[3]);
  ESP_LOGE("PAR", "LIMITER: %d %d %d %d %d %d %d %d",
           settings.par_limiter_threshold[0], settings.par_limiter_threshold[1],
           settings.par_limiter_threshold[2], settings.par_limiter_threshold[3],
           settings.par_limiter_release[0], settings.par_limiter_release[1],
           settings.par_limiter_release[2], settings.par_limiter_release[3]);
  ESP_LOGE("PAR", "Delay: %d %d %d %d", settings.delay[0], settings.delay[1],
           settings.delay[2], settings.delay[3]);
  ESP_LOGE("PAR", "BassEnhance: %d %d", settings.bassenhance[0],
           settings.bassenhance[1]);
  ESP_LOGE("PAR", "DynBass: %d %d %d %d %d", settings.dynbass[0],
           settings.dynbass[1], settings.dynbass[2], settings.dynbass[3],
           settings.dynbass[4]);
  ESP_LOGE("PAR", "Bypass Global: %d %d %d %d %d", settings.global_bypass[0],
           settings.global_bypass[1], settings.global_bypass[2],
           settings.global_bypass[3], settings.global_bypass[4]);

  for (int channel = 0; channel < 4; channel++) {

    ESP_LOGE("PAR", "LP CH%d: %d %d %d", channel, settings.par_lp[channel][0],
             settings.par_lp[channel][1], settings.par_lp[channel][2]);
    ESP_LOGE("PAR", "HP CH%d: %d %d %d", channel, settings.par_hp[channel][0],
             settings.par_hp[channel][1], settings.par_hp[channel][2]);

    for (int i = 0; i < 5; i++) {
      ESP_LOGE("PAR", "EQ CH%d: %d %d %d", channel,
               settings.par_eq[channel][i * 3 + 0],
               settings.par_eq[channel][i * 3 + 1],
               settings.par_eq[channel][i * 3 + 2]);
    }

    ESP_LOGE("PAR", "Bypass CH%d: %d %d %d %d %d %d %d", channel,
             settings.channel_bypass[channel][0],
             settings.channel_bypass[channel][1],
             settings.channel_bypass[channel][2],
             settings.channel_bypass[channel][3],
             settings.channel_bypass[channel][4],
             settings.channel_bypass[channel][5],
             settings.channel_bypass[channel][6]);
  }

  for (int i = 0; i < 8; i = i + 2) {
    ESP_LOGE("PAR", "IR Addr: %d CMD: %d", settings.irparams[i],
             settings.irparams[i + 1]);
  }
}
...
void initDSPFilters(float fsin) {
  ...
  SetGain(settings.par_channel_gains[0], settings.par_channel_gains[1],
          settings.par_channel_gains[2], settings.par_channel_gains[3]);
  SetLimiter(
      settings.par_limiter_threshold[0], settings.par_limiter_threshold[1],
      settings.par_limiter_threshold[2], settings.par_limiter_threshold[3],
      settings.par_limiter_release[0], settings.par_limiter_release[1],
      settings.par_limiter_release[2], settings.par_limiter_release[3]);
  SetMute(settings.mute[0], settings.mute[1], settings.mute[2],
          settings.mute[3]);
  SetPolarity(settings.polarity[0], settings.polarity[1], settings.polarity[2],
              settings.polarity[3]);
  SetDelay(settings.delay[0], settings.delay[1], settings.delay[2],
           settings.delay[3]);
  SetBassEnhance(settings.bassenhance[0], settings.bassenhance[1]);
  SetDynBass(settings.dynbass[0], settings.dynbass[1], settings.dynbass[2],
             settings.dynbass[3], settings.dynbass[4]);
  SetGlobalBypass(settings.global_bypass[0], settings.global_bypass[1],
                  settings.global_bypass[2], settings.global_bypass[3],
                  settings.global_bypass[4]);

  for (int channel = 0; channel < 4; channel++) {
    SetLowPass(channel, settings.par_lp[channel][0],
               settings.par_lp[channel][1], settings.par_lp[channel][2]);
    SetHighPass(channel, settings.par_hp[channel][0],
                settings.par_hp[channel][1], settings.par_hp[channel][2]);

    for (int i = 0; i < 5; i++) {
      SetEQ(channel, i, settings.par_eq[channel][i * 3 + 0],
            settings.par_eq[channel][i * 3 + 1],
            settings.par_eq[channel][i * 3 + 2]);
    }

    SetChannelBypass(channel, settings.channel_bypass[channel][0],
                     settings.channel_bypass[channel][1],
                     settings.channel_bypass[channel][2],
                     settings.channel_bypass[channel][3],
                     settings.channel_bypass[channel][4],
...
void SetChannelBypass(int channel, int lp, int hp, int eq0, int eq1, int eq2,
                      int eq3, int eq4) {
  if (channel < 0 || channel > 3)
    return;
  if (lp < 0 || lp > 1)
...
void GetChannelBypass(int channel, int *lp, int *hp, int *eq0, int *eq1,
                      int *eq2, int *eq3, int *eq4) {
...
void SetPolarity(int channel0, int channel1, int channel2, int channel3) {
  if (channel0 < 0 || channel0 > 1)
    return;
  if (channel1 < 0 || channel1 > 1)
    return;
  if (channel2 < 0 || channel2 > 1)
    return;
  if (channel3 < 0 || channel3 > 1)
    return;
...
void SetDelay(int channel0, int channel1, int channel2, int channel3) {
  if (channel0 < 0 || channel0 > 3000)
    return;
  if (channel1 < 0 || channel1 > 3000)
    return;
  if (channel2 < 0 || channel2 > 3000)
    return;
  if (channel3 < 0 || channel3 > 3000)
    return;
  ...
  for (int i = 0; i < 4; i++) {
    if (delay_w_ptr[i] < 0. || delay_w_ptr[i] > 1500)
      delay_w_ptr[i] = 0;
  }
  delay_r_ptr = 0;
  ESP_LOGI("DELDEL", "%d %d %d %d", delay_w_ptr[0], delay_w_ptr[1],
           delay_w_ptr[2], delay_w_ptr[3]);
}
...
void SetIRParams(int addr_onoff, int cmd_onoff, int addr_volup, int cmd_volup,
                 int addr_voldown, int cmd_voldown, int addr_mute,
                 int cmd_mute) {
...
void GetIRParams(int *addr_onoff, int *cmd_onoff, int *addr_volup,
                 int *cmd_volup, int *addr_voldown, int *cmd_voldown,
                 int *addr_mute, int *cmd_mute) {
