...
static uint8_t GetSurviveMode() { return survivedata & 0x000000ff; }
...
void SetBluetoothReady() { IsBluetoothActive = 1; }
...
static void IR_TaskHandler(void *arg) {
  ...
  ir_parser_config_t ir_parser_config =
      IR_PARSER_DEFAULT_CONFIG((ir_dev_t)example_rx_channel);
    ...
    if (items) {
      ...
      if (ir_parser->input(ir_parser, items, length) == ESP_OK) {
        if (ir_parser->get_scan_code(ir_parser, &addr, &cmd, &repeat) ==
            ESP_OK) {
          GetIRParams(&irparams[0], &irparams[1], &irparams[2], &irparams[3],
                      &irparams[4], &irparams[5], &irparams[6], &irparams[7]);
          ESP_LOGI("IR", "Scan Code %s --- addr: 0x%04x cmd: 0x%04x",
          ...
          if (irparams[2] == addr && irparams[3] == cmd &&
              GetSurviveMode() != 2) {
            ESP_LOGE("SYSMGT", "VOLUP");
            VolumeUp();
          }
          if (irparams[4] == addr && irparams[5] == cmd &&
              GetSurviveMode() != 2) {
            ESP_LOGE("SYSMGT", "VOLDOWN");
            VolumeDown();
          }
          if (irparams[6] == addr && irparams[7] == cmd && repeat == 0 &&
              GetSurviveMode() != 2) {
...
static void ButtonLEDPowerTask(void *arg) {
  ...
  while (1) {
    currentbitmask = (gpio_get_level(25) << 5) | (gpio_get_level(33) << 4) |
                     (gpio_get_level(23) << 3) | (gpio_get_level(13) << 2) |
                     (gpio_get_level(18) << 1) | gpio_get_level(17);
    // ESP_LOGI("last", "%d", lastbitmask);
    // ESP_LOGI("current", "%d", currentbitmask);

    if (false && lastbitmask == emptybitmask && currentbitmask == 0b00011111) {
    ...
    } else if (lastbitmask == emptybitmask && currentbitmask == 0b0001111 &&
               GetSurviveMode() != 2) {
      ESP_LOGI("SW", "DSP Button pressed");
      WriteSurvive(GetSurviveVolume(), 1);
      gpio_set_level(12, 0);
      esp_restart();
    } else if (lastbitmask == emptybitmask && currentbitmask == 0b0010111 &&
               GetSurviveMode() != 2) {
      ESP_LOGI("SW", "VOL UP Button pressed");
      VolumeUp();
    } else if (lastbitmask == emptybitmask && currentbitmask == 0b0011011 &&
               GetSurviveMode() != 2) {
      ESP_LOGI("SW", "VOL down Button pressed");
      VolumeDown();
    } else if (lastbitmask == emptybitmask && currentbitmask == 0b0011101 &&
               GetSurviveMode() != 2) {
      ESP_LOGI("SW", "mute Button pressed");
      MuteUnMute();
    } else if (lastbitmask == emptybitmask && currentbitmask == 0b0011110 &&
               GetSurviveMode() != 2) {
...
void initSysmgmt(uint8_t *mode, uint8_t *initaudiodevice) {
  ...
  io_conf3.pin_bit_mask = ((1ULL << 25) | (1ULL << 17) | (1ULL << 13) |
                           (1ULL << 18) | (1ULL << 33) | (1ULL << 23));
  // gpio_set_pull_mode(GPIO_NUM_35, GPIO_PULLUP_ENABLE);
  // gpio_set_pull_mode(GPIO_NUM_33, GPIO_PULLUP_ENABLE);
  // gpio_set_pull_mode(GPIO_NUM_17, GPIO_PULLUP_ENABLE);
  ...
  xTaskCreatePinnedToCore(IR_TaskHandler, "ir_rx_task", 2048, NULL,
                          configMAX_PRIORITIES - 3, NULL, 0);
  xTaskCreatePinnedToCore(ButtonLEDPowerTask, "power_task", 2048, NULL,
                          configMAX_PRIORITIES - 3, NULL, 0);
...
void PowerUpMA120x0() { gpio_set_level(12, 1); }
